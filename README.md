# Hi, I'm Muhammet Sait Doğmuş 👋

Computer Engineer focused on **AI, computer vision, LLMs, RAG, backend development and automation**.

I build production-ready applications using **Python, C#, ASP.NET Core, .NET MAUI and SQL**.

## Featured Projects

- **Eye Tracking + Emotion Analysis** — Real-time gaze tracking, off-screen detection and emotion analysis with OpenCV, MediaPipe and ONNX.
- **Tam Vakti** — Android productivity app published on Google Play.
- **DryCar Care** — Live appointment and business management platform.
- **Hand Mouse** — Computer vision-based virtual mouse using hand tracking.

## AI & Core Stack

`Python` · `LLMs` · `RAG` · `OpenCV` · `MediaPipe` · `FastAPI` · `C#` · `ASP.NET Core` · `.NET MAUI` · `SQL`

Some of my AI projects are private and not publicly available on GitHub.
